...
{
	std::ofstream fmap("./src/Test_ver1.map");
	if (!fmap.is_open())
		{
...
void Compute::createped()
{
	...
	std::ofstream fped("./src/Test_ver1.ped");
	if (!fped.is_open())
	{
...
void Compute::sample2vec()
{
	std::ifstream samplefile ("./src/Test.sample");//Test_7_Columns.sample
...
void Compute::gen2vec()
{

	std::ifstream genfile ("./src/Test.gen");
	Gen gen;
	std::string line;
	std::string dnaseq;
	std::stringstream ss;
